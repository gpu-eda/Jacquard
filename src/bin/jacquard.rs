...
enum Commands {
    ...
    /// Emit a ready-to-use OpenOCD `remote_bitbang` config for
    /// `cosim --jtag-server`.
    ///
    /// Writes a working `openocd.cfg` (remote_bitbang driver + a RISC-V
    /// Debug TAP + target) so you don't have to hand-write the
    /// boilerplate — the most error-prone part of attaching a debugger.
...
#[derive(Parser)]
struct JtagOpenocdConfigArgs {
    /// remote_bitbang TCP port — the one `cosim --jtag-server` logs
    /// (`listening on 127.0.0.1:<port>`). With `--jtag-server 0` the port
    /// is OS-assigned, so fill this in after the server prints it.
    #[clap(long, default_value = "9999")]
    port: u16,

    /// Host the server listens on (cosim binds 127.0.0.1).
    #[clap(long, default_value = "localhost")]
    host: String,

    /// TAP instruction-register length. 5 is the RISC-V Debug TAP
    /// default; check your DTM if it differs.
    #[clap(long, default_value = "5")]
    irlen: u8,

    /// Optional expected JTAG IDCODE (e.g. `0xdeadbeef`). When set,
    /// OpenOCD validates the TAP against it.
    #[clap(long)]
    expected_id: Option<String>,

    /// TAP/target chip name used in the generated config.
    #[clap(long, default_value = "jacquard")]
    chipname: String,

    /// Seconds for OpenOCD's RISC-V DMI command timeout. The GPU server is
    /// per-edge (slow) while stepping, so keep it generous.
...
fn cmd_jtag_openocd_config(args: JtagOpenocdConfigArgs) {
    ...
    let expected_id = args
        .expected_id
        .as_deref()
        .map(|id| format!(" -expected-id {id}"))
        .unwrap_or_default();
    let cfg = format!(
        "# SPDX-License-Identifier: Apache-2.0\n\
         # OpenOCD config generated by `jacquard jtag-openocd-config`.\n\
         # Drives a Jacquard `cosim --jtag-server` over remote_bitbang.\n\
         # Start the server, then: openocd -f openocd.cfg\n\
         \n\
         adapter driver remote_bitbang\n\
         remote_bitbang host {host}\n\
         remote_bitbang port {port}\n\
         transport select jtag\n\
         # The Jacquard server has no notion of clock speed (the client\n\
         # paces every bit), so adapter speed is nominal.\n\
         adapter speed 1\n\
         \n\
         set _CHIPNAME {chipname}\n\
         jtag newtap $_CHIPNAME cpu -irlen {irlen}{expected_id}\n\
         set _TARGETNAME $_CHIPNAME.cpu\n\
         target create $_TARGETNAME riscv -chain-position $_TARGETNAME\n\
         \n\
         # GPU cosim steps one bitbang transaction per edge while a client\n\
         # is attached — keep the DMI timeout generous.\n\
         riscv set_command_timeout_sec {dmi_timeout}\n\
         \n\
         init\n\
         # Bare DTM+DM fixtures with no real hart will stop here at MISA;\n\
...
fn cmd_cosim(args: CosimArgs) {
    {
        ...
        let design_args = DesignArgs {
            ...
            clock_period_ps,
            xprop: args.xprop,
            xprop_undriven_inputs: args.xprop,
            liberty: None,
            timing_ir: args.timing_ir.clone(),
            timing_corner: None,
            cell_library: args.cell_library.clone(),
