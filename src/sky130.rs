...
// `SKY130_PIN_TABLE: &[(&str, &[(&str, Direction)])]`. NOT YET WIRED
// into the runtime lookup path. The hand-rolled `direction_of` arms
// below disagree with this generated table on 171 (cell, pin) pairs
// spanning 42 cell types — most are state cells the hand-rolled table
// never enumerates (`dfbbn`, `dfxbp`, `sdfrtp`, etc., all of which
// have `Q_N` outputs and several of which use `CLK_N`), plus
// arm-ordering bugs (e.g. `t.starts_with("buf")` shadows
// `t.starts_with("bufinv")`, so `bufinv_*` cells' `Y` pin would
// currently panic in `direction_of`).
//
// The ignored test `generated_pin_table_agrees_with_hand_rolled`
// in this module enumerates the divergences. Reconciling them is
// a follow-up: it likely means switching the SKY130 consumer to
// exact lookup (like gf180mcu) and dropping the prefix-match arms.
// Until then, this static is informational only; allow `dead_code`
// because the equivalence test gates the only consumer.
#[allow(dead_code)]
mod generated {
...
mod tests {
    ...
    /// Cross-check the build.rs-generated `SKY130_PIN_TABLE` against the
    /// hand-rolled `direction_of` arms. For every (cell_type, pin) entry
    /// in the generated table, the hand-rolled lookup must return the
    /// same direction.
    ///
    /// Currently this test fails on 171 (cell, pin) pairs across 42
    /// cell types — see the comment above the `mod generated` block.
    /// Marked `#[ignore]` until the SKY130 consumer is switched over
    /// to exact lookup (the gf180mcu pattern). At that point this test
    /// becomes the regression gate.
    ///
    /// Run explicitly with
    /// `cargo test --lib generated_pin_table_agrees_with_hand_rolled -- --ignored --nocapture`.
    #[test]
    #[ignore = "SKY130 hand-rolled direction_of has 171 known divergences from \
                the generated table; enable once the consumer is switched to \
                exact lookup."]
    fn generated_pin_table_agrees_with_hand_rolled() {
        use super::generated::SKY130_PIN_TABLE;
        let provider = SKY130LeafPins;
        let mut total_pins = 0usize;
        let mut total_cells = 0usize;
        let mut mismatches: Vec<String> = Vec::new();
        for (cell_type, pins) in SKY130_PIN_TABLE {
            total_cells += 1;
            // Construct a representative macro name (drive-0 hd variant)
            // so the prefix-stripper in `extract_cell_type` recovers
            // exactly `cell_type`. Drive strength is irrelevant for the
            // hand-rolled lookup — it matches on the stripped base.
            let macro_name: CompactString =
                format!("sky130_fd_sc_hd__{cell_type}_0").into();
            for (pin_name, expected_dir) in *pins {
                total_pins += 1;
                let actual = std::panic::catch_unwind(|| {
                    provider.direction_of(
                        &macro_name,
                        &CompactString::from(*pin_name),
                        None,
                    )
                });
                match actual {
                    Ok(dir) if dir == *expected_dir => {}
                    Ok(dir) => mismatches.push(format!(
                        "{cell_type}::{pin_name}: generated={expected_dir:?} \
                         hand-rolled={dir:?}",
                    )),
                    Err(_) => mismatches.push(format!(
                        "{cell_type}::{pin_name}: generated={expected_dir:?} \
                         hand-rolled=PANIC (unknown pin)",
                    )),
                }
            }
        }
        if !mismatches.is_empty() {
            panic!(
                "{} of {} (cell, pin) pairs from SKY130_PIN_TABLE disagree with \
                 the hand-rolled lookup:\n{}",
                mismatches.len(),
                total_pins,
                mismatches.join("\n"),
            );
        ...
        eprintln!(
            "SKY130_PIN_TABLE: {total_cells} cell types, {total_pins} (cell, pin) \
             pairs all agree with hand-rolled SKY130LeafPins::direction_of.",
        );
    }
}
