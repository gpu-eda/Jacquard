...
pub struct CosimOpts {
    ...
    /// TCP port for an interactive `remote_bitbang` JTAG server. When
    /// set alongside a `jtag` peripheral, cosim binds the port, blocks
    /// for one client (OpenOCD), and drives the configured pins live —
...
fn run_cosim_generic<B: CosimBackend>(
    ...
    if let Some(jtag_cfg) = config.jtag.as_ref() {
        // Pin resolution is shared by both byte sources (replay file and
        // live `--jtag-server` socket); only the source differs. TCK/TMS/
        // TDI/TRST are design *inputs* (resolved against `input_bits`);
        // TDO is a design *output* (resolved against `output_bits`).
        ...
        let model: Option<Box<dyn PeripheralModel>> =
            match (opts.jtag_replay.as_ref(), opts.jtag_server) {
                ...
                (Some(replay_path), None) => {
                    ...
                    clilog::info!(
                        "JTAG replay `jtag_0`: {stream_len} stream bytes from {}, \
                         hold_edges={} ({pins})",
                        replay_path.display(),
                        opts.jtag_hold_cycles
                    );
                ...
                // Config present, no source flag: inputs would float.
                (None, None) => {
                    clilog::warn!(
                        "sim_config.json declares a `jtag` peripheral but neither \
                         --jtag-replay <PATH> nor --jtag-server <PORT> was \
                         supplied; JTAG inputs will float"
                    );
                    None
                }
            };
        if let Some(model) = model {
            register_model(model, &mut models, &mut model_driven_positions);
        }
    } else if opts.jtag_replay.is_some() || opts.jtag_server.is_some() {
        panic!(
            "--jtag-replay/--jtag-server supplied but sim_config.json has no \
             `jtag` peripheral entry to bind it to"
        );
    }
