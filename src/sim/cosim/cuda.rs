...
//! CUDA GPU backend for co-simulation (Stage B/C — design step + GPU UART/bus +
//! SPI flash peripherals): `CudaBackend` + the `run_cosim_cuda` shim. Gated
...
impl CosimBackend for CudaBackend {
    fn new(
        aig: &AIG,
        netlistdb: &NetlistDB,
        ...
        if script.xprop_enabled {
            ...
            clilog::info!(
                "cosim X-propagation enabled (CudaBackend): {} reg/io words, \
        ...
        // reaches here; this stays for layout parity with `sim_cuda`.
        ...
        if let Some(sram_init) = config.sram_init.as_ref() {
            ...
            .unwrap_or_else(|e| panic!("sram_init: {e}"));
            clilog::info!(
                "SRAM preload (CudaBackend): {} bytes from {} → cell {} at storage offset {}",
                total_bytes,
                elf_path.display(),
                cellid,
                storage_offset
...
pub fn run_cosim_cuda(
    ...
    run_cosim_generic::<CudaBackend>(design, config, opts, timing_constraints)
}
