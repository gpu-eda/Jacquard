...
**Status:** Proposed — not started. Tracks
[#124](https://github.com/gpu-eda/Jacquard/issues/124).
...
| # | Scope | Gate |
|---|---|---|
| J1 | Wire `output_state` into `step_edge` (D2); add `JtagConfig.tdo_gpio` + resolve `tdo_pos`; unit-test TDO sampling in the replay model | replay fixtures still byte-identical; TDO-sample unit test green |
| J2 | Byte-source abstraction (D3): `JtagSource` enum; replay path refactored onto it | `jtag_minimal` replay unchanged; model unit tests green |
| J3 | `--jtag-server` (D5) + live byte source + `R` write-back (D4) + `TcpListener` accept | **V1 loopback test**: live run == replay golden (`data0_obs==0xCAFEBABE`) |
| J4 | `docs/jtag-debug.md` (V2 recipe); `--help` text; cross-link ADRs | docs build; manual OpenOCD/gdb smoke (local) |
| J5 (later) | Single-step / breakpoints via DM `step`/triggers; X-aware debug under `--xprop` | follows from attach; see open questions |
