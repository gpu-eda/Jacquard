# Decision 0011 — RAM port-mapping schema for declarative cell metadata
...
[Decision 0010](0010-declarative-cell-metadata.md) shipped a minimal Tier 2
...
Decision 0010 § "Deferred to a future ADR" listed the port-mapping
schema explicitly:
...
- **Built-in classifier removal.** Same rule as Decision 0010 — the
  `$__RAMGEM_SYNC_` and `CF_SRAM_*` recognisers stay as fallback;
  manifest-declared RAMs supplement, don't replace.

## Links

- Decision 0010 — Declarative cell metadata (the parent decision deferring
  this schema).
