# Decision 0017 — Cosim execution model
...
1. **Peripheral ring buffers** — UART channels and Wishbone trace
   ...
   `write_head`. See Decision 0013 for struct conventions.
...
- **This makes the contract's "observe" half real.** The Decision section and
  Decision 0013 both note `step_edge` is handed an **empty `output_state`** today
...
- **Cosim `--timing-report` (structured JSON) is not wired.** The structured
  report is `sim`-only; see Decision 0008's 2026-06-25 amendment. The cosim path
  emits arrival-annotated VCD but not the JSON report.

## Cross-references

- Decision 0012 — CDC jitter injection (uses the scheduler's edge
  timestamps as the injection point).
- Decision 0013 — Peripheral model architecture (documents GPU-side
