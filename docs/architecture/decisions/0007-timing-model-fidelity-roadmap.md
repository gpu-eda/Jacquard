# Decision 0007 — Timing model fidelity roadmap
...
Jacquard's timing model today consumes SDF-equivalent annotations via the timing IR (Decision 0002), produced and validated by OpenSTA called out of process (Decision 0001 — sole STA path; Decision 0003's in-process OpenTimer alternative was Superseded by the spike). The accuracy contract at present is "±5% on arrival times against CVC reference" per `timing-validation.md`. This is acceptable for sky130-class designs at ≥10 ns clock periods.
...
Adopt a three-pillar roadmap for closing the fidelity gap with CVC, while preserving Jacquard's GPU-throughput advantage. All three pillars are consumer-side work (`src/flatten.rs`, `src/aig.rs`, `src/sim/cosim_metal.rs`, the kernel arrival math); none require schema changes inconsistent with Decision 0002 nor abandoning the cycle-accurate boomerang kernel architecture.
...
- The IR schema may be extended (additive only) per Decision 0002 to carry pillar-specific data. Extensions require a minor schema bump and a documented consumer-version compatibility note.
...
- `crates/timing-ir/schemas/timing_ir.fbs` accumulates additive extensions for clock arrival and per-cell δ(T) parameters. Schema versioning per Decision 0002 governs.
