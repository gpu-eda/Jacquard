# Decision 0020 — Python engine as a bundled binary wheel
...
**Relates to:** [Decision 0018](0018-distribution-and-installation.md) (distribution
...
channels — the binary (`brew` / `cargo binstall`, per Decision 0018) **and** the
...
1. **The binary is self-contained *except* for a runtime-library tail.** The
   `sim` Metal kernel is embedded via `include_bytes!` (Decision 0018 / v0.2.1), so
...
4. **Phase the platform matrix** (`docs/plans/...`):
   ...
   - **P2 — Linux/x86_64 CPU fallback.** The cosim CPU backend (Decision 0017) with
     no GPU runtime, so `pip install` works in plain CI containers.
   - **P3 — CUDA / HIP.** Gated on Decision 0018 Phase 4 (prebuilt GPU binaries) and
