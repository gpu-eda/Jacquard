# Decision 0014 — AIG as simulation intermediate representation
...
reduction tree (Decision 0015) can execute them all with a single GPU
...
| RuntimeCellLibrary | User-supplied cell metadata (Decision 0010) for cells outside vendored PDKs |
...
- The boomerang reduction tree (Decision 0015) works because every node is
