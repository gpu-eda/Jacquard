# Decision 0006 — SDF preprocessing model and interim-to-release cutover
...
- The interface is arms-length: standard EDA interchange formats (Liberty / Verilog / SDF / SPEF / SDC) in, our own IR JSON (Decision 0002) out. No shared data structures, no headers, no linking.
...
2. Probe OpenSTA's version on first invocation and fail with a remediation message if it is older than the version pinned in `vendor/opensta/` (per Decision 0005).
...
Concurrently, Decision 0001 establishes OpenSTA as the timing correctness oracle (subprocess, never linked, GPL), and Decision 0002 introduces a timing IR that decouples parsing from consumption.
...
- **Native Rust SDF→IR converter** replaces the OpenSTA subprocess call inside `jacquard sim input.sdf`. Grammar-based (nom / pest), validated against OpenSTA on the corpus per Decision 0001.
...
- **If OpenSTA becomes unmaintainable or disappears** — the submodule pin (Decision 0005) remains authoritative for the integrated version. A forked submodule can maintain any necessary patches.
...
- Decision 0001 — OpenSTA as oracle (subprocess model).
- Decision 0002 — timing IR (format consumed).
- Decision 0005 — OpenSTA vendoring (submodule for reproducibility + stress corpus).
