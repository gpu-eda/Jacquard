...
rates](../adr/README.md#three-kinds-of-claim-aged-differently): why a decision
...
0003](../adr/0003-opentimer-primary-sta.md) already cites the
...
The [ADR README](../adr/README.md) currently says ADRs are *living documents,
...
- `dec-0022-flow-controlled-io.md` — a decision-log entry (was ADR 0022).

The number is a stable ID, not a reading order. Decision entries keep their
existing four-digit ADR numbers, so every "ADR 0022" reference already in the
...
`docs/adr/` and `docs/plans/` carry the class; the file inside is just
